...
#+BEGIN_QUOTE
[!NOTE]
Screenshots are a bit out of date at the moment due to changes being done. This might take a few days to sort out due to outside things needing to be done as well. See [[https://github.com/themkat/.emacs.d/issues/27][issue 27]] for details. Peace and long life! <3
#+END_QUOTE
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Recentf file list need to contain WAAAAY more history. Useful for Helm-mini
...
**** Shading languages
I have probably used glsl-mode through Try everytime I need it for like 10 years now... Time to actually have it permanently. (I'm lazy okay, don't judge)
#+begin_src emacs-lisp
...
**** Julia
I've recently tried learning Julia to replace Python. While Python is great with NumPy, it sometimes feels a bit unintuitive. Matlab/Octave is slightly more intuitive on some math syntax (e.g, matrices, column vectors vs row vectors etc.), but comes with its own flaws. Julia seems really exciting! It also have one of the features I really miss in scripting languages like Python: the ability to specify types WHEN I WANT TO! (yeah, I know, Python has type hints, but those are not enforced at runtime). Intuitive vectors, types when needed, and macros! What's not to love? :heart_eyes_cat:
...
**** Flutter and Dart
Lately I've been dipping my toes into making apps with Flutter with a friend of mine. That calls for some Flutter related setup, which I've been getting using Try for a while now.
